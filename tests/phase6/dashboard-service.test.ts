...
describe('DashboardService.getStats', () => {
  test('returns zeroes when no campaigns exist', () => {
  ...
  test('counts campaigns by status', () => {
    const { campaignService, dashboard } = createDashboard();

    campaignService.createCampaign({ title: 'Draft', videoAssetId: 'a1' });
    const { campaign: c2 } = campaignService.createCampaign({ title: 'Ready', videoAssetId: 'a2' });
    campaignService.addTarget(c2.id, { channelId: 'ch-1', videoTitle: 'V', videoDescription: 'D' });
  ...
  test('calculates target success rate', () => {
    const { campaignService, dashboard } = createDashboard();

    const { campaign } = campaignService.createCampaign({ title: 'Mixed', videoAssetId: 'a1' });
    const { target: t1 } = campaignService.addTarget(campaign.id, { channelId: 'ch-1', videoTitle: 'V1', videoDescription: 'D1' });
    const { target: t2 } = campaignService.addTarget(campaign.id, { channelId: 'ch-2', videoTitle: 'V2', videoDescription: 'D2' });
    const { target: t3 } = campaignService.addTarget(campaign.id, { channelId: 'ch-3', videoTitle: 'V3', videoDescription: 'D3' });
    ...
    campaignService.updateTargetStatus(campaign.id, t3.id, 'erro', { errorMessage: 'quota' });
  ...
  test('aggregates per-channel metrics', () => {
    const { campaignService, dashboard } = createDashboard();

    // Campaign 1: ch-main succeeds, ch-backup fails
    const { campaign: c1 } = campaignService.createCampaign({ title: 'C1', videoAssetId: 'a1' });
    const { target: t1 } = campaignService.addTarget(c1.id, { channelId: 'ch-main', videoTitle: 'V1', videoDescription: 'D' });
    const { target: t2 } = campaignService.addTarget(c1.id, { channelId: 'ch-backup', videoTitle: 'V2', videoDescription: 'D' });
    campaignService.updateTargetStatus(c1.id, t1.id, 'publicado', { youtubeVideoId: 'yt-1' });
    campaignService.updateTargetStatus(c1.id, t2.id, 'erro', { errorMessage: 'fail' });

    // Campaign 2: ch-main succeeds again
    const { campaign: c2 } = campaignService.createCampaign({ title: 'C2', videoAssetId: 'a2' });
    const { target: t3 } = campaignService.addTarget(c2.id, { channelId: 'ch-main', videoTitle: 'V3', videoDescription: 'D' });
    campaignService.updateTargetStatus(c2.id, t3.id, 'publicado', { youtubeVideoId: 'yt-3' });
  ...
  test('includes job statistics', () => {
    const { campaignService, jobService, dashboard } = createDashboard();

    const { campaign } = campaignService.createCampaign({ title: 'Jobs', videoAssetId: 'a1' });
    const { target: t1 } = campaignService.addTarget(campaign.id, { channelId: 'ch-1', videoTitle: 'V1', videoDescription: 'D1' });
    const { target: t2 } = campaignService.addTarget(campaign.id, { channelId: 'ch-2', videoTitle: 'V2', videoDescription: 'D2' });
  ...
  test('counts retries across jobs', () => {
    const { campaignService, jobService, dashboard } = createDashboard();

    const { campaign } = campaignService.createCampaign({ title: 'Retries', videoAssetId: 'a1' });
    const { target } = campaignService.addTarget(campaign.id, { channelId: 'ch-1', videoTitle: 'V', videoDescription: 'D' });
